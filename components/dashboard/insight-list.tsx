  ...
  return (
    <div className="flex flex-col gap-3">
      {items.map((insight) => {
        ...
        return (
          <div key={insight.id} className="flex gap-3 rounded-lg border border-border/60 bg-muted/30 p-3">
            <div className={cn("flex size-8 shrink-0 items-center justify-center rounded-md", className)}>
              <Icon className="size-4" />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-medium">{insight.title}</p>
              <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground text-pretty">{insight.body}</p>
            </div>
          </div>
        )
      })}
